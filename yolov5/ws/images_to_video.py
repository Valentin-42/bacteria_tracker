...
def image_to_video(path,video_ouput_path,video_name) :
    print("image to video '%s' -> '%s' '%s'"%(path,video_ouput_path,video_name))
    ...
    file_names = sorted([os.path.join(path, fn) for fn in os.listdir(path) if fn.endswith('.jpg')])
    if len(file_names)==0:
        print("No files...")
        return
    ...
    video = cv2.VideoWriter(os.path.join(video_ouput_path,video_name), cv2.VideoWriter_fourcc(*'MJPG'), 10, (width,height))
